...
Oczekiwane pola:
- `generated_at`
- `queue.{total,due_now,pending,processing,sent,failed}`
- `runtime.{is_running,ticks_total,processed_total,retried_total,dead_letter_total,last_error}`
- `config.{poll_ms,batch_size,max_attempts,retry_base_ms,retry_max_ms}`
